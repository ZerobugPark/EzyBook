...
final class DefaultNetworkService: NetworkService, MultiPartNetworkSevice {
    ...
    func fetchMultiPartData<T: Decodable & EntityConvertible, R: NetworkRouter>(dto: T.Type ,_ router: R) async throws -> T {
        ...
        guard let multipartBuilder = router.multipartFormData else {
               throw APIError(localErrorType: .missingEndpoint)//APIError(localErrorType: .invalidMultipart)
           }
        
        let response = await session.upload(multipartFormData: multipartBuilder, to: router.endpoint!, method: router.method, headers: router.headers)
            .validate(statusCode: 200...299)
            .serializingData()
            .response
